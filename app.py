...
def _spawn_request_from_schedule(schedule):
    ...
    total_amount, components_json, provider_payout, admin_fee, site_fee = build_request_costing(
...
def create_schedule():
    try:
        ...
        end_date = _parse_date(data.get('end_date'))  # None = run until manually stopped

        # Snapshot the order details
        components_data = data.get('components_data', [])
        components_json = None
        if components_data:
            components_json = [{
        ...
        # Preview amount for the response
        total_amount, _, _, _, _ = build_request_costing(service, components_data)
...
def update_schedule(schedule_id):
    try:
        ...
        # Recompute next run from (possibly) new config
