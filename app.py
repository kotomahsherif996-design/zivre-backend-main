...
class ServiceRequest(db.Model):
    ...
    schedule_id = db.Column(db.Integer, db.ForeignKey('scheduled_services.id', ondelete='SET NULL'), nullable=True, index=True)
    # Number of days this request's amount covers (1 = single visit; >1 for pre-paid recurring periods)
    period_days = db.Column(db.Integer, nullable=False, default=1)


class ScheduledService(db.Model):
    ...
    # Recurrence config
    schedule_type = db.Column(db.String(20), nullable=False, default='one_time')  # one_time | daily | weekly | monthly
    weekly_day = db.Column(db.Integer, nullable=True)    # 0=Mon ... 6=Sun (weekly only)
    monthly_date = db.Column(db.Integer, nullable=True)  # 1..28 (monthly only)
    start_date = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)
    end_date = db.Column(db.DateTime, nullable=True)     # null = run until manually stopped
    ...
    # Number of days each spawned request should be charged for:
    # daily=1, weekly=7, monthly=days in that month, one_time custom range=days between start/end
    period_days = db.Column(db.Integer, nullable=False, default=1)

    # Self-contained snapshot of the order (so spawned requests don't depend on later edits)
    components_data = db.Column(db.JSON, nullable=True)
    location_address = db.Column(db.String(200), nullable=False, default='')
    location_city = db.Column(db.String(100), nullable=False, default='')
    location_region = db.Column(db.String(100), nullable=False, default='')
...
def build_request_costing(service, components_data):
    """Single source of truth for request amount, components snapshot, and fee split.

    Returns: (total_amount, components_json, provider_payout, admin_fee, site_fee)
    Mirrors the exact logic that create_request used previously so behavior is unchanged.
    """
    ...
    return total_amount, components_json, provider_payout, admin_fee, site_fee
...
def send_verification_email(user_email, user_name, verification_token):
    try:
        ...
        # ----- DEBUG PRINT (ADD THESE TWO LINES RIGHT HERE) -----
        print(f"BREVO_API_KEY exists: {bool(os.environ.get('BREVO_API_KEY'))}")
        print(f"BREVO_API_KEY prefix: {os.environ.get('BREVO_API_KEY', '')[:15]}...")
        # ---------------------------------------------------------
...
def update_profile(user_id):
    ...
    return jsonify({'message': 'Profile updated successfully', 'user': {
        'id': user.id, 'full_name': user.full_name, 'phone': user.phone, 'email': user.email, 'avatar': user.avatar
    }})
...
def delete_service(service_id):
    try:
        ...
        # Safety: block deletion if there are active (non-terminal) requests for this service
        active_statuses = ['pending_approval', 'assigned', 'in_progress', 'completed']
        ...
        if active_count > 0:
            return jsonify({
                'error': f'Cannot delete: {active_count} active request(s) use this service. Deactivate it instead.'
            }), 400

        # Deactivate any active schedules referencing this service
        ScheduledService.query.filter_by(service_id=service_id, is_active=True).update(
            {'is_active': False, 'status': 'cancelled', 'next_run': None}
        )
        ...
        try:
            socketio.emit('service_deleted', {'id': service_id, 'name': service_name})
        except Exception as e:
            print(f"service_deleted emit error (non-critical): {e}")
...
def create_request():
    try:
        ...
        # Calculate amount + fees via shared helper (single source of truth)
        components_data = data.get('components_data', [])
        total_amount, components_json, provider_payout, admin_fee, site_fee = build_request_costing(service, components_data)
        ...
        for r in requests:
            try:
                result.append({
                    ...
                    'amount': r.amount,
                    'period_days': r.period_days or 1,
...
def get_available_jobs():
    ...
    return jsonify([{
        ...
        'amount': req.amount,
        'period_days': req.period_days or 1,
...
def get_provider_jobs(provider_id):
    ...
    return jsonify([{
        ...
        'status': j.status,
        'amount': j.amount,
        'period_days': j.period_days or 1,
        ...
        'is_recurring': j.schedule_id is not None,
...
def delete_user(user_id):
    try:
        ...
        # ✅ CRITICAL: Clear referrer_id first
        User.query.filter_by(referrer_id=user_id).update({'referrer_id': None, 'position': None})
        
        # Force flush to apply referrer_id changes before delete
        db.session.flush()
        ...
        # Delete referral-related records
        try:
        ...
        # ✅ Force another flush to clear all pending deletes
        db.session.flush()
        ...
        # ✅ IMPORTANT: Close the session to prevent stale connections
        db.session.close()
...
def init_db():
    ...
    # ============================================
    # SCHEDULED SERVICES - AUTO CREATE TABLE & COLUMN
    # ============================================
    try:
        db.session.execute(text('''
            CREATE TABLE IF NOT EXISTS scheduled_services (
                ...
                location_address VARCHAR(200) DEFAULT '',
                location_city VARCHAR(100) DEFAULT '',
                location_region VARCHAR(100) DEFAULT '',
        ...
        '''))
        db.session.execute(text('CREATE INDEX IF NOT EXISTS idx_scheduled_services_user_id ON scheduled_services(user_id)'))
        db.session.execute(text('CREATE INDEX IF NOT EXISTS idx_scheduled_services_next_run ON scheduled_services(next_run)'))
        db.session.execute(text('CREATE INDEX IF NOT EXISTS idx_scheduled_services_status ON scheduled_services(status)'))
        db.session.execute(text('ALTER TABLE service_requests ADD COLUMN IF NOT EXISTS schedule_id INTEGER REFERENCES scheduled_services(id) ON DELETE SET NULL'))
        db.session.execute(text('CREATE INDEX IF NOT EXISTS idx_service_requests_schedule_id ON service_requests(schedule_id)'))
        # Safe additive migrations for already-deployed databases (existing rows default to period_days=1,
        # which is correct: a single, non-recurring visit).
        db.session.execute(text('ALTER TABLE scheduled_services ADD COLUMN IF NOT EXISTS period_days INTEGER NOT NULL DEFAULT 1'))
        db.session.execute(text('ALTER TABLE service_requests ADD COLUMN IF NOT EXISTS period_days INTEGER NOT NULL DEFAULT 1'))
        db.session.commit()
        print("✅ Scheduled services table & schedule_id column ready")
    except Exception as e:
        db.session.rollback()
        print(f"⚠️ Scheduled services migration note: {e}")
...
SCHEDULER_ADVISORY_LOCK_KEY = 918273645  # arbitrary, app-unique


def _add_one_month(dt):
    """Add one calendar month, capping the day at 28 to avoid month-overflow."""
    month = dt.month + 1
...
def compute_next_run(schedule, after):
    """Return the next run datetime strictly after `after`, or None if the
    schedule should not run again (one_time already used, or past end_date)."""
    stype = schedule.schedule_type

    if stype == 'one_time':
        return None  # one_time runs exactly once at next_run; never re-armed

    nxt = schedule.next_run or schedule.start_date or after
    # Advance until strictly in the future relative to `after`
    guard = 0
    while nxt <= after and guard < 1000:
        if stype == 'daily':
...
def _spawn_request_from_schedule(schedule):
    """Create a real ServiceRequest from a schedule template using shared costing.

    The schedule stores a raw components snapshot (component_id, name, quantity, price) —
    exactly the same shape build_request_costing expects for a normal one-time request.
    That gives us the PER-DAY amount. We then multiply by period_days (computed once when
    the schedule was created: daily=1, weekly=7, monthly=days in that month,
    custom range=days between start/end) to get the amount actually charged for this spawn.
    """
    service = db.session.get(Service, schedule.service_id)
    if not service or not service.is_active:
        return None

    period_days = schedule.period_days or 1

    per_day_amount, per_day_components_json, _, _, _ = build_request_costing(
        service, schedule.components_data
    )
    total_amount = per_day_amount * period_days

    # Scale the components breakdown for display while keeping quantity × unit_price = subtotal true.
    # daily_unit_price is kept alongside so the UI can show "GHS{daily_unit_price}/day × {period_days} days".
    components_json = None
    if per_day_components_json:
        components_json = [{
            'component_id': item.get('component_id'),
            'name': item.get('name'),
            'quantity': item.get('quantity'),
            'daily_unit_price': item.get('unit_price'),
            'unit_price': round((item.get('unit_price') or 0) * period_days, 2),
            'subtotal': round((item.get('subtotal') or 0) * period_days, 2),
            'period_days': period_days
        } for item in per_day_components_json]
    ...
    new_request = ServiceRequest(
        ...
        location_city=schedule.location_city,
        location_region=schedule.location_region,
        location_landmark=schedule.location_landmark,
    ...
    create_notification(
        schedule.user_id,
        f'Your scheduled request for {service.name} has been submitted automatically.',
    ...
    try:
        ...
        print(f"Schedule emit error (non-critical): {e}")
...
def process_due_schedules():
    ...
    for sched in due:
        try:
            ...
            _spawn_request_from_schedule(sched)
...
def _acquire_scheduler_lock():
    """Try to grab a single-runner lock so only one worker runs the loop.
    On Postgres uses advisory lock; on SQLite/others, returns True (single proc)."""
    try:
        uri = app.config.get('SQLALCHEMY_DATABASE_URI', '')
        if uri.startswith('postgres'):
            res = db.session.execute(
                text('SELECT pg_try_advisory_lock(:k)'), {'k': SCHEDULER_ADVISORY_LOCK_KEY}
            ).scalar()
            db.session.commit()
            return bool(res)
        return True  # local/sqlite dev: single process
    except Exception as e:
        print(f"Scheduler lock check failed (assuming single runner): {e}")
        return True


def scheduler_loop():
    """Background loop: only the lock-holding worker actually ticks."""
    with app.app_context():
        if not _acquire_scheduler_lock():
            print("⏭️  Scheduler lock held by another worker; this worker will not tick.")
            return
...
# Start the recurrence engine as a single background task
try:
    socketio.start_background_task(scheduler_loop)
except Exception as e:
    print(f"Could not start scheduler background task: {e}")
...
# ==================== SCHEDULED SERVICES ROUTES ====================

def _parse_date(value):
    """Accept ISO date/datetime string -> datetime, or None."""
    if not value:
        return None
    try:
        v = value.replace('Z', '')
...
def _compute_first_run(schedule_type, start_date, weekly_day, monthly_date):
    """Compute the first next_run from the recurrence config."""
    now = datetime.utcnow()
    base = start_date or now
    if base < now:
        base = now
    ...
    if schedule_type == 'weekly':
        wd = weekly_day if weekly_day is not None else base.weekday()
        days_ahead = (wd - base.weekday()) % 7
        return base + timedelta(days=days_ahead)
    if schedule_type == 'monthly':
        md = min(max(monthly_date or base.day, 1), 28)
        candidate = base.replace(day=md)
        if candidate < base:
...
def _compute_period_days(schedule_type, start_date, end_date, weekly_day, monthly_date):
    """Single source of truth for how many days a schedule's amount should cover.

    daily = 1, weekly = 7, monthly = days in the month of the first run,
    one_time (custom range) = days between start_date and end_date inclusive (else 1).
    """
    if schedule_type == 'daily':
        return 1
    if schedule_type == 'weekly':
        return 7
    if schedule_type == 'monthly':
        first_run = _compute_first_run(schedule_type, start_date, weekly_day, monthly_date)
        return _calendar.monthrange(first_run.year, first_run.month)[1]
    if schedule_type == 'one_time':
...
def _schedule_pricing_preview(s, service=None):
    """Compute (per_day_amount, total_amount) for a schedule using the same raw
    components snapshot format used everywhere else (build_request_costing)."""
    if service is None:
        service = db.session.get(Service, s.service_id)
    if service is None:
        return 0, 0
    per_day_amount, _components_json, _pp, _af, _sf = build_request_costing(service, s.components_data)
    period_days = s.period_days or 1
    return per_day_amount, per_day_amount * period_days


def _serialize_schedule(s):
    service = db.session.get(Service, s.service_id)
    per_day_amount, total_amount = _schedule_pricing_preview(s, service)
    return {
        'id': s.id,
        ...
        'monthly_date': s.monthly_date,
        'start_date': s.start_date.isoformat() if s.start_date else None,
        'end_date': s.end_date.isoformat() if s.end_date else None,
        'next_run': s.next_run.isoformat() if s.next_run else None,
        'status': s.status,
        'is_active': s.is_active,
        'runs_count': s.runs_count or 0,
        'last_run_at': s.last_run_at.isoformat() if s.last_run_at else None,
        'components_data': s.components_data,
        'location_city': s.location_city,
        'location_region': s.location_region,
        'created_at': s.created_at.isoformat() if s.created_at else None,
        'period_days': s.period_days or 1,
        'per_day_amount': round(per_day_amount, 2),
...
def create_schedule():
    try:
        ...
        if schedule_type not in ('one_time', 'daily', 'weekly', 'monthly'):
            return jsonify({'error': 'Invalid schedule_type'}), 400

        weekly_day = data.get('weekly_day')
        monthly_date = data.get('monthly_date')
        if schedule_type == 'weekly' and weekly_day is None:
            return jsonify({'error': 'weekly_day is required for weekly schedules'}), 400
        if schedule_type == 'monthly' and monthly_date is None:
            return jsonify({'error': 'monthly_date is required for monthly schedules'}), 400
        if monthly_date is not None:
            monthly_date = min(max(int(monthly_date), 1), 28)

        start_date = _parse_date(data.get('start_date')) or datetime.utcnow()
        end_date = _parse_date(data.get('end_date'))

        period_days = _compute_period_days(schedule_type, start_date, end_date, weekly_day, monthly_date)

        # Sanitize + snapshot components in the SAME raw shape create_request uses
        # (component_id, name, quantity, price) so _spawn_request_from_schedule can
        # reuse build_request_costing without any format conversion/mismatch.
        raw_components_in = data.get('components_data') or []
        components_to_store = []
        for item in raw_components_in:
            try:
        ...
        components_to_store = components_to_store or None
        ...
        sched = ScheduledService(
            ...
            weekly_day=weekly_day if schedule_type == 'weekly' else None,
            monthly_date=monthly_date if schedule_type == 'monthly' else None,
            start_date=start_date,
            end_date=end_date,
            next_run=first_run,
            status='active',
            is_active=True,
            components_data=components_to_store,
            location_address=data.get('location_address', ''),
            location_city=data.get('location_city', ''),
            location_region=data.get('location_region', ''),
            location_landmark=data.get('location_landmark', ''),
        ...
        # Calculate preview total using the same shared costing helper as everywhere else
        per_day_amount, total_amount = _schedule_pricing_preview(sched, service)

        # ⚠️ CRITICAL: Serialize schedule BEFORE processing due schedules
        # This reads all attributes while the object is still attached to the session
        schedule_data = _serialize_schedule(sched)

        # Now process due schedules (this may commit and detach the object)
        process_due_schedules()

        return jsonify({
            'message': 'Schedule created successfully',
            'schedule': schedule_data,  # Use the pre-serialized data
            'amount': total_amount,
            'period_days': period_days
        })
    except Exception as e:
...
@app.route('/api/schedules/mine', methods=['GET'])
@token_required
def get_my_schedules():
    try:
        schedules = ScheduledService.query.filter_by(
            user_id=request.current_user.id
        ).order_by(ScheduledService.created_at.desc()).all()
        return jsonify([_serialize_schedule(s) for s in schedules])
    except Exception as e:
        print(f"Error in get_my_schedules: {str(e)}")
        return jsonify({'error': str(e)}), 500
...
def get_all_schedules():
    if request.current_user.role != 'admin':
        return jsonify({'error': 'Admin access required'}), 403
    try:
        schedules = ScheduledService.query.order_by(ScheduledService.created_at.desc()).all()
        result = []
        for s in schedules:
            data = _serialize_schedule(s)
            customer = db.session.get(User, s.user_id)
            data['customer_name'] = customer.full_name if customer else 'Unknown'
            data['customer_phone'] = s.customer_phone
            result.append(data)
        return jsonify(result)
    except Exception as e:
...
def update_schedule(schedule_id):
    try:
        sched = db.session.get(ScheduledService, schedule_id)
        if not sched:
            return jsonify({'error': 'Schedule not found'}), 404
        if sched.user_id != request.current_user.id:
            return jsonify({'error': 'You can only edit your own schedules'}), 403

        data = request.json or {}

        if 'schedule_type' in data:
            st = data['schedule_type']
            if st not in ('one_time', 'daily', 'weekly', 'monthly'):
                return jsonify({'error': 'Invalid schedule_type'}), 400
            sched.schedule_type = st
        if 'weekly_day' in data:
            sched.weekly_day = data['weekly_day']
        if 'monthly_date' in data and data['monthly_date'] is not None:
            sched.monthly_date = min(max(int(data['monthly_date']), 1), 28)
        if 'start_date' in data:
            sched.start_date = _parse_date(data['start_date']) or sched.start_date
        if 'end_date' in data:
            sched.end_date = _parse_date(data['end_date'])  # may be None to clear
        for f in ('location_address', 'location_city', 'location_region', 'location_landmark', 'customer_phone'):
            if f in data:
                setattr(sched, f, data[f])

        # Recompute next run and period_days from (possibly) new config
        sched.next_run = _compute_first_run(
            sched.schedule_type, sched.start_date, sched.weekly_day, sched.monthly_date
        )
        sched.period_days = _compute_period_days(
            sched.schedule_type, sched.start_date, sched.end_date, sched.weekly_day, sched.monthly_date
        )
        db.session.commit()
        return jsonify({'message': 'Schedule updated', 'schedule': _serialize_schedule(sched)})
    except Exception as e:
        db.session.rollback()
...
@app.route('/api/schedules/<int:schedule_id>/pause', methods=['PUT'])
@token_required
def pause_schedule(schedule_id):
    ...
    sched.is_active = False
    db.session.commit()
    return jsonify({'message': 'Schedule paused', 'schedule': _serialize_schedule(sched)})


@app.route('/api/schedules/<int:schedule_id>/resume', methods=['PUT'])
@token_required
def resume_schedule(schedule_id):
    ...
    if sched.status in ('completed', 'cancelled'):
        return jsonify({'error': f'Cannot resume a {sched.status} schedule'}), 400
    sched.status = 'active'
    sched.is_active = True
    # Re-arm next_run to the next valid point from now
    sched.next_run = _compute_first_run(
        sched.schedule_type, datetime.utcnow(), sched.weekly_day, sched.monthly_date
    )
    db.session.commit()
    return jsonify({'message': 'Schedule resumed', 'schedule': _serialize_schedule(sched)})
...
def cancel_schedule(schedule_id):
    ...
    sched.next_run = None
    db.session.commit()
    return jsonify({'message': 'Schedule cancelled', 'schedule': _serialize_schedule(sched)})
